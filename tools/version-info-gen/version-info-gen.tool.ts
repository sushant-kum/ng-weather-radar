...
const options = cli
  .option(
    "-t, --tag <version-tag>",
    "version tag for generation of version-info files",
    version
  )
...
console.info(
  `${chalk.gray("[VIG]")} Wrote${
    options.dev ? chalk.cyanBright(" dev") : ""
  } version info ${chalk.blueBright(versionInfo.version)} to ${chalk.blueBright(
    relative(resolve(__dirname, "..", ".."), envFile)
  )}`
);
...
writeFileSync(assetJsonFile, `${JSON.stringify(versionInfo, null, 2)}`, {
  encoding: "utf-8",
});
...
console.info(
  `${chalk.gray("[VIG]")} Wrote${
    ...
    relative(resolve(__dirname, "..", ".."), assetJsonFile)
  )}`
);
